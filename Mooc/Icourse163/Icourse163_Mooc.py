...
class Icourse163_Mooc(Icourse163_Base):
    ...
    signature_url = 'https://www.icourse163.org/web/j/resourceRpcBean.getResourceToken.rpc?csrfKey='
    infos_data = {
        'callCount':'1', 
    ...
    # TODO 字幕获取展示还没支持
    def _get_video_url_new(self, params):
        # 获取csrfKey
        cookies = request_cookies()
        for cookie in cookies:
            if cookie.name == 'NTESSTUDYSI':
                csrfKey = cookie.value
        if not (csrfKey):
            return None

        # 获取signature
        url = self.signature_url + csrfKey
        data = {
            'bizId': params[2],
            'bizType': '1',
            'contentType': params[1]
        }
        rsp = request_post(url=url, data=data)
        signature = json.loads(rsp).get('result').get('videoSignDto').get('signature')
        if not (signature):
            return None
        ...
        url = 'https://vod.study.163.com/eds/api/v1/vod/video?' + urlencode(data)
        rsp = request_get(url)
        
        video_url = sub_url = None
        videos = json.loads(rsp).get('result').get('videos')
        if not (videos):
            return None
